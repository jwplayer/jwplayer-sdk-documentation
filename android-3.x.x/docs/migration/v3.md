...
Your 2.x license key will not work with the JW Player SDK for Android 3.0 and above. You can get your new mobile-only license key from the JW Player Dashboard under Players > Downloads and Keys. You may currently use Maven to update to 3.x, or you can download AAR files from the Dashboard in the same location as the license key.
